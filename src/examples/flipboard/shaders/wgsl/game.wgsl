...
fn main(@builtin(local_invocation_id) localInvocationID: vec3<u32>, @builtin(workgroup_id) workgroupID: vec3<u32>) {
    ...
    let id = tex_coord.y * i32(constants.game_res.x) + tex_coord.x;
    ...
    if(a > MIN_FLAP_VAL) {                                                              // if during flap
        if(a == 1.0){                                                                   // down
            ...
            let new_val = min(abs(prev_val + s * constants.time_delta), 1.0) * s;         // increase val
            game_output[id] = vec2(new_val);
        }
    } else {
        let uv = vec2<f32>(tex_coord.xy) / constants.game_res;
        var clr = sign(get_pattern(uv) - 0.5);
        if(s != clr) {
            let new_val = prev_val + s * constants.time_delta;                            // start flap
            game_output[id] = vec2(new_val);
        }
    }
}
...
fn get_pattern(in_uv: vec2<f32>) -> f32 {
    ...
    return step(distance(in_uv, vec2(0.5 + sin(constants.time * 0.25) * 0.25, 0.5)), .25);
}